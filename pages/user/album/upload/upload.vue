	...
	<view class="padding-lr-sm">
		<u-form :model="model" :rules="rules" ref="uForm" :errorType="errorType">
			<u-form-item :label-position="labelPosition" label="相册" label-width="100" prop="album">
				<u-input :border="border" type="select" :select-open="albumShow" v-model="model.album"
					placeholder="请选择相册" @click="albumShow = true"></u-input>
			</u-form-item>
					...
					if (valid) {
						...
						if (images && images.length > 0) {
							...
							try {
								...
								this.$util.helper.modal('上传成功')
							} catch (e) {
								uni.hideLoading();
							}
						} else {
							this.$util.helper.modal('请选择图片')
						}
					}
				});
			}
