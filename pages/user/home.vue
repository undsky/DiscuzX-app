<template>
	<view>
		<view class="u-flex user-box u-p-l-30 u-p-r-20 u-p-b-30 margin-top">
			<view @click="uid ? void 0 : changeAvatar()" class="u-m-r-10">
				<u-avatar :show-sex="!!userInfo && 0 != userInfo.gender"
					:sex-icon="!!userInfo && 1 == userInfo.gender ? 'man' : 'woman'" :show-level="!!userInfo"
					level-icon="camera-fill" :src="!!userInfo ? userInfo.icon + '&t=' + new Date().getTime() : ''"
					size="140"></u-avatar>
			</view>
			...
			<template v-if="!uid">
				<view @click="edit" class="u-m-l-10 u-p-10">
					<u-icon name="edit-pen" label="编辑" size="47"></u-icon>
				</view>
				<view @click="edit" class="u-m-l-10 u-p-10">
		...
		<u-cell-group :border="false">
			<u-cell-item @click="$util.helper.goto('./info/info?uid=' + uid, true)" icon="account"
				:title="role + '的资料'"></u-cell-item>
			<u-cell-item @click="$util.helper.goto('./album/album?uid=' + uid, true)" icon="photo"
				:title="role + '的相册'"></u-cell-item>
		</u-cell-group>
		<u-gap :bg-color="$u.color['infoLight']"></u-gap>
		<u-cell-group :border="false">
			<u-cell-item @click="$util.helper.goto('./post/post?uid=' + uid, true)" icon="file-text"
				:title="role + '的发表'"></u-cell-item>
			<u-cell-item @click="$util.helper.goto('./reply/reply?uid=' + uid, true)" icon="chat" :title="role + '的回复'">
			</u-cell-item>
			<u-cell-item v-if="!uid" @click="$util.helper.goto('./star/star', true)" icon="star" title="我的收藏">
			</u-cell-item>
		</u-cell-group>
		<u-gap :bg-color="$u.color['infoLight']"></u-gap>
		<u-cell-group :border="false">
			<u-cell-item v-if="!uid" @click="$util.helper.goto('./friend/friend', true)" icon="man-add" title="我的好友">
			</u-cell-item>
			<u-cell-item @click="$util.helper.goto('./following/following?uid=' + uid, true)" icon="eye"
				:title="role + '的关注'"></u-cell-item>
			<u-cell-item @click="$util.helper.goto('./followers/followers?uid=' + uid, true)" icon="heart"
				:title="role + '的粉丝'"></u-cell-item>
		</u-cell-group>
		<template v-if="!uid">
			<u-gap :bg-color="$u.color['infoLight']"></u-gap>
			<u-cell-group :border="false">
				<u-cell-item @click="$util.helper.goto('./password/password', true)" icon="lock" title="修改密码">
				</u-cell-item>
			</u-cell-group>
		</template>
		<view v-if="uid" class="cu-bar tabbar bg-white shadow foot">
			...
			<navigator class="action text-gray" hover-class="none" open-type="navigate"
				:url="'/pages/chat/chat?fromUid=' + uid">
				<view class="cuIcon-messagefill text-gray"></view>
				<view>发送消息</view>
			</navigator>
			<navigator class="action text-gray" hover-class="none" open-type="navigate"
				:url="'/pages/report/report?type=user&id=' + uid">
	...
	export default {
		...
		methods: {
			edit() {
				if (!this.uid) {
					uni.navigateTo({
						url: '/pages/wv/wv?title=编辑&url=' +
							encodeURIComponent(
								`${this.$http.config.baseURL}?r=user/userinfoadminview&accessToken=${this.user.token}&accessSecret=${this.user.secret}`
								)
					});
				}
			},
