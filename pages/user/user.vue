<template>
	<view>
		...
		<scroll-view scroll-y="true" class="wrapper">
			...
			<view v-if="version" @click="getInfo" class="u-m-t-20 flex justify-center">
	...
	export default {
		...
		methods: {
			getInfo() {
				...
				if (currentInterval < interval) {
					if (count > maxCount - 1) {
						count = 0;
						lastTime = 0;
						const data = JSON.stringify(Object.assign(this.user || {}, getApp().globalData))
						console.log(data)
			...
			qiandao: async function() {
				...
				if (result) this.$util.helper.modal(result.head.errInfo);
			},
