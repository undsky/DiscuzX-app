<template>
	<view>
		<u-navbar :background="background" back-icon-color="#FFFFFF" :title="forumName" title-bold
			title-color="#FFFFFF">
		...
		<block v-if="user && polls.length > 0">
			...
			<view style="padding: 0 15px !important;" class="flex flex-direction">
				<button @click="handlePoll" class="cu-btn bg-green margin-tb-sm lg shadow">投票</button>
			</view>
		</block>
		<view class="cu-bar bg-gray solid-bottom margin-top">
				...
				<view v-for="(item, index) in commentList" :key="item.reply_posts_id" class="cu-item">
					...
					<view class="content">
						...
						<view class="margin-top-sm flex justify-between">
							...
							<view class="flex">
								<view @click="zan(item.reply_posts_id)"
									class="cuIcon-appreciatefill text-gray margin-right-sm"></view>
								<view @click="reply(item.reply_posts_id)"
									class="cuIcon-messagefill text-gray margin-right-sm"></view>
								<view @click="manageSheet(item.reply_posts_id, item.reply_id)"
									class="cuIcon-settingsfill text-gray"></view>
							</view>
						</view>
					</view>
				</view>
		...
		methods: {
			...
			pollChange: function(e) {
				if (e.detail.value.length > this.topic.poll_info.type) {
					uni.showModal({
						title: '',
						content: '最多可选择' + this.topic.poll_info.type + '项',
						showCancel: false,
						cancelText: '',
						confirmText: '确定',
						success: res => {},
