				...
				this.$refs.uForm.validate(async valid => {
					if (valid) {
						...
						try {
							...
							if (music && music.src) {
								const audioResult = await this.$http.uploadAttachment(music.src, 'forum',
									'audio');
								body.push({
									type: 3,
									infor: audioResult.body.attachment[0].urlName
								});
								aid.push(audioResult.body.attachment[0].id);
							}
