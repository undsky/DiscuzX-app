<template>
	<view>
		...
		<swiper class="swiper" :current="currentIndex" @transition="transition" @animationfinish="animationfinish">
			...
			<swiper-item>
				<scroll-view class="sv" scroll-y="true" @scrolltoupper="reloadPhoto" @scrolltolower="loadMorePhoto">
...
<script>
	...
	export default {
			...
			loadMorePhoto: async function() {
				if (this.allowLoad && 'nomore' != this.loadStatus) {
					...
					try {
						...
						if (result) {
							this.photoList.push(...result.list.map(item => {
								item.last_reply_date = this.$u.timeFrom(item.last_reply_date)
								return item
							}));
		...
		onShow: async function() {
			if (this.user && 0 == this.photoList.length)
				await this.reloadPhoto()
		}
	};
</script>
